...
def plugin_payloads(plugin):
    if plugin.payload_category == "SQLI":
        payloads = plugin.get_payloads(aggressive=False)
        if SAFE_MODE:
...
class SmartScanner:
    ...
    async def run_sqli_plugin(self, client, plugin, capability):
        ...
        semaphore = asyncio.Semaphore(sqli_parallel_limit())
        candidates = []

        async def run_one(payload, param=None):
            async with semaphore:
                if SAFE_MODE and SQLI_PAYLOAD_DELAY_SEC:
    ...
    async def run_check(self, capability, plugin, client=None):
        params = self.target_parameters(capability, plugin)
        timeout = self.check_timeout_for(plugin)

        try:
            if client is None:
                async with Client(self.target) as owned:
    ...
    async def _run_check_with_client(self, client, capability, plugin, timeout):
        if plugin.name == "Information Disclosure":
    ...
    async def scan_stream(self):
        ...
        try:
            async with Client(self.target) as client:
                engine = DiscoveryEngine(self.target, client=client)
