...
from autopwn import run_autopwn, run_autopwn_stream
from fastmcp import Client
from mcp_client import enumerate_server, probe_server
...
def expand_template(template, values):
    names = list(dict.fromkeys(re.findall(r"{([^}]+)}", template)))
    missing = [name for name in names if not str(values.get(name, "")).strip()]

    if missing:
        raise ValueError(f"Missing template values: {', '.join(missing)}")

    return template.format(**{name: str(values[name]).strip() for name in names})


async def read_resource_at(url, uri):
    async with Client(url) as client:
        result = await client.read_resource(uri)
        return render_result_text(result)


async def read_template_at(url, template, values):
    uri = expand_template(template, values)
    return await read_resource_at(url, uri)


async def call_tool_at(url, name, arguments):
    async with Client(url) as client:
        result = await client.call_tool(name, arguments)
        return render_result_text(result)


async def run_prompt_at(url, name, arguments):
    async with Client(url) as client:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/recon":
            try:
                result = asyncio.run(run_recon(url))
        ...
        if path == "/api/autopwn":
            if payload.get("stream"):
                self._send_ndjson_stream(run_autopwn_stream(url))
                return
            try:
                result = asyncio.run(run_autopwn(url))
                self._send_json(result)
            except Exception as exc:
                self._send_json({"ok": False, "error": str(exc)}, 500)
            return
        ...
        if path == "/api/read-resource":
            ...
            if not uri:
                ...
                return
            self._run_action(read_resource_at(url, uri))
            return
        ...
        if path == "/api/read-template":
            ...
            if not template:
                ...
                return
            self._run_action(read_template_at(url, template, values))
            return
        ...
        if path == "/api/call-tool":
            ...
            if not name:
                ...
                return
            self._run_action(call_tool_at(url, name, arguments))
            return
        ...
        if path == "/api/run-prompt":
            ...
            if not name:
                self._send_json({"ok": False, "error": "Missing name"}, 400)
                return
            self._run_action(run_prompt_at(url, name, arguments))
            return
