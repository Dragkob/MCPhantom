...
class SQLiPlugin(Plugin):
    ...
    def looks_like_system_variable_string(self, value, payload):
        ...
        combined = payload.lower() + " " + unquote(payload).lower()
        if "@@hostname" in combined:
            return bool(re.fullmatch(r"[a-zA-Z0-9._-]{1,253}", text))
        if "@@basedir" in combined:
            return text.startswith(("/", "\\")) and len(text) > 1
        if "@@version_comment" in combined:
            return len(text) >= 3
    ...
    def extract_version_output(self, text, payload=""):
        stripped = text.strip()
        if not stripped:
            return None
            ...
            for row in rows:
                for value in row.values():
                    ...
                    if self.looks_like_version_string(candidate):
                        return candidate
                    if payload and self.looks_like_system_variable_string(candidate, payload):
                        return candidate
        ...
        for line in lines[:6]:
            if self.looks_like_version_string(line):
                return line
            if payload and self.looks_like_system_variable_string(line, payload):
                return line
    ...
    def looks_like_version_leak(self, payload, text, baseline=""):
        ...
        return self.extract_version_output(text, payload) is not None

    def _normalize_for_fingerprint(self, text, payload=""):
        stripped = text.strip()
        if not stripped:
            return ""
        ...
        for needle in needles:
            if needle:
                stripped = re.sub(re.escape(needle), "", stripped, flags=re.IGNORECASE)

        stripped = re.sub(
            r'(?i)unknown resource:\s*["\']?[^"\']+["\']?',
            "unknown resource:<uri>",
            stripped,
        )
        stripped = re.sub(r"(?i)[a-z][a-z0-9+.-]*://[^\s\"']+", "<uri>", stripped)
        stripped = re.sub(r"(?i)(price|quantity|filecount|password|getfile)://\S+", "<uri>", stripped)
        return re.sub(r"\s+", " ", stripped).strip()

    def _data_marker_fingerprint(self, payload, text):
        lower = self.effective_error_text(payload, text)
    ...
    @staticmethod
    def extract_raw_response(evidence):
        marker = "Raw response:\n"
        ...
        table_idx = body.find("Table view:\n")
        if table_idx != -1:
            body = body[:table_idx]
        return body.strip()

    def finding_dedup_key(self, finding):
        raw = self.extract_raw_response(finding.evidence)
    ...
    def result_fingerprint(self, text, baseline="", payload=""):
        stripped = self._normalize_for_fingerprint(text, payload)
        if not stripped:
            return None

        if baseline and stripped == self._normalize_for_fingerprint(baseline, payload):
            return None
        ...
        lower = stripped.lower()
        if lower.startswith("unknown resource:"):
            return "error-prefix:unknown resource"

        marker_fp = self._data_marker_fingerprint(payload, text)
        if marker_fp:
            return marker_fp
    ...
    def build_finding(self, capability, payload, text, level, baseline=""):
        indicator = self.matched_indicator(payload, text)
        version_output = self.extract_version_output(text, payload) if self.is_version_payload(payload) else None

        if self.looks_like_version_leak(payload, text, baseline):
            if any(
                needle in unquote(payload).lower() + payload.lower()
                for needle in ("@@hostname", "@@basedir", "@@version_comment")
            ):
                title = "SQL Injection - Database Metadata Leaked"
                hint = f"System variable output: {version_output}"
            else:
